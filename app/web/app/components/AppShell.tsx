...
export default function AppShell({ active, actions, header, children }: { active: Section; actions?: ReactNode; header?: ReactNode; children: ReactNode }) {
  ...
  const serviceState = dataServiceStatus(serviceHealth.data, serviceHealth.isError);
  ...
  function prefetchSection(section: Section) {
    if (section === active) return Promise.resolve();
    ...
    switch (section) {
      ...
      case "spu-audience":
        return queryClient.prefetchQuery(spuAssetsQueryOptions());
      case "tasks":
