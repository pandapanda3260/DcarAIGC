...
type ContentForm = { id: number | null; platform: string; platformContentId: string; canonicalUrl: string; publishedAt: string; title: string; body: string; contentType: string; accountUid: string; accountName: string; accountType: string; contentDirection: string };
const emptyForm: ContentForm = { id: null, platform: "douyin", platformContentId: "", canonicalUrl: "", publishedAt: "", title: "", body: "", contentType: "unknown", accountUid: "", accountName: "", accountType: "unknown", contentDirection: "unknown" };
...
export default function ContentsPage() {
  ...
  function edit(item?: ContentItem) {
    setForm(item ? { id: item.id, platform: item.platform, platformContentId: item.platform_content_id ?? "", canonicalUrl: item.canonical_url, publishedAt: item.published_at?.slice(0, 16) ?? "", title: item.title, body: item.body ?? "", contentType: item.content_type ?? "unknown", accountUid: item.raw_account_uid ?? "", accountName: item.raw_account_name ?? "", accountType: item.account_type, contentDirection: item.content_direction } : { ...emptyForm });
  }
  async function save() {
    if (!form) return;
    setSaving(true); feedback("");
    try {
      const body = { platform: form.platform, platform_content_id: form.platformContentId || null, canonical_url: form.canonicalUrl, published_at: form.publishedAt ? new Date(form.publishedAt).toISOString() : null, title: form.title, body: form.body, content_type: form.contentType, account_uid: form.accountUid, account_name: form.accountName, account_type: form.accountType, content_direction: form.contentDirection };
      await readJson(form.id ? `/api/v8/contents/${form.id}` : "/api/v8/contents", jsonRequest(body, form.id ? "PATCH" : "POST"));
      const wasEdit = Boolean(form.id); setForm(null); await reload(); feedback("", wasEdit ? "内容已更新" : "内容已新增");
