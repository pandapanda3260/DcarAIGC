  ...
  return <ContentDialog title="内容详情" busy={saving} onClose={onClose} status={status} footer={<>
    ...
    <button type="button" className="secondary" disabled={saving || updateLocked} title={updateLocked ? updateLabel : undefined} onClick={onEdit}>修改</button>
        ...
        <section className={styles.detailSection}><h3>分类与评估</h3><dl className={styles.detailGrid}>
          <div><dt>账号分组</dt><dd>{accountGroupLabel(item.account_group)}</dd></div>
          <div><dt>业务方向</dt><dd>{businessDirectionLabel(item.business_direction)}</dd></div>
          <div><dt>作品内容方向</dt><dd>{label(item.content_direction)}</dd></div>
            ...
            <table className={styles.table} aria-label={`${day}发布内容`}>
              ...
              <tbody>{group.map((item) => {
                ...
                <td className={styles.classificationCell}><dl className={styles.classification}>
                  <div><dt>账号分组</dt><dd data-known={item.account_group && item.account_group !== "unknown" ? "true" : undefined}>{accountGroupLabel(item.account_group)}</dd></div>
                  <div><dt>业务方向</dt><dd data-known={item.business_direction && item.business_direction !== "unknown" ? "true" : undefined}>{businessDirectionLabel(item.business_direction)}</dd></div>
                </dl></td>
