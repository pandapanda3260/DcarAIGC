...
test("the three custom shell headers keep their title, description and existing actions", async () => {
  for (const [path, eyebrow, title, description, actions] of [
    ["contents/ContentsPage.tsx", "内容资料库", "发布内容明细", "更新数据时会同步更新详情、指标以及已保存的视频和图片；重复提醒会指向最早发布的内容。", ["ContentExportControl"]],
    ["accounts/AccountsPage.tsx", ["系统账号", "矩阵通名册"], "账号信息", "未采集的粉丝和平台作品总量显示“—”。", ["新增系统账号", "批量上传账号", "下载账号表格"]],
