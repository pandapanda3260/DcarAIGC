#!/usr/bin/env python3
"""Prepare a discovery_metrics successor retaining every historical proof; no DB/service/provider writes.
...
Existing authorizations and their issue/expiry times are inherited unchanged.
"""
...
def packaging(checkout):
    ...
    spec = importlib.util.spec_from_file_location("daily_pipeline_packaging_primitives", path)
...
def prepare(args):
    ...
    successor = {"contract": module.CONTRACT, "transition": module.TRANSITION,
        ...
        **{field: inherited[name]["proof_sha256"] for field, name in module.INHERITED_PROOFS.items()}}
    build = {**parent, "source_root": str(args.source_root), "git": tree["git"],
        ...
        "discovery_metrics_successor": successor, "created_at": at,
        "validation_scope": "reviewed discovery_metrics delta; complete recovery, profile, Publisher, controls, metric, catalog and historical operator proofs inherited"}
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fixture.chmod(0o600)
        bootstrap = base.load(args.source_root / "src/dcar_eval/v8/runtime_paths.py", "daily_pipeline_frozen_bootstrap")
    ...
    result = {"contract": "discovery-metrics-install-proposal-v1", "status": "prepared", "created_at": at,
        ...
        **{field: proof[name]["proof_sha256"] for field, name in module.INHERITED_PROOFS.items()},
        "discovery_metrics_proof_sha256": proof["discovery_metrics_proof"]["proof_sha256"],
