#!/usr/bin/env python3
"""Prepare the reviewed schema21 manual-content code update without DB writes.
...
Neither action changes services, formal database, provider gates or qualification.
"""
...
base = load(ROOT / "scripts/prepare_account_classification_release.py", "manual_release_packaging_primitives")
write, write_bytes, inventory = base.write, base.write_bytes, base.inventory


def verified_inputs(args):
    module = load(args.checkout / MODULE, "reviewed_manual_release")
...
def prepare(args):
    ...
    build = {**parent, "source_root": str(args.source_root), "git": tree["git"],
        ...
        "manual_content_scope_successor": successor, "created_at": at,
        "validation_scope": "reviewed schema21 manual-content code fix; original capture authority retained"}
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fixture.chmod(0o600)
        bootstrap = load(args.source_root / "src/dcar_eval/v8/runtime_paths.py", "manual_release_frozen_bootstrap")
    ...
    result = {"contract": "manual-content-scope-install-proposal-v1", "status": "prepared", "created_at": at,
