#!/usr/bin/env python3
"""Import a frozen official Dongchedi series catalog (dry-run by default)."""
...
from v8.spu_series_catalog_import import (  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="已冻结并带 catalog_sha256 的官方车系目录 JSON",
    )
    parser.add_argument(
        "--db",
        type=Path,
        default=DEFAULT_DB,
        help="目标 schema-v15 SQLite 数据库（默认本地正式库）",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
        help="执行写入；缺省仅生成只读 dry-run 计划",
    )
    parser.add_argument(
        "--expect-plan-sha256",
        default=None,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    except (SpuSeriesCatalogImportError, OSError, sqlite3.Error) as error:
        print(
            json.dumps(
                {
                    "schema_version": "dcar-dongchedi-spu-series-import-error-v1",
                    "ok": False,
                    "error": str(error),
                },
                ensure_ascii=False,
