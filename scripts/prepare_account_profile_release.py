#!/usr/bin/env python3
"""Prepare an account-profile successor retaining every historical proof; no DB/service/provider writes.
...
def packaging(checkout):
    ...
    spec = importlib.util.spec_from_file_location("account_profile_packaging_primitives", path)
...
def prepare(args):
    ...
    authorization_ref = base.write(args.evidence_root / "profile-authorization.json", {
        "contract": module.AUTHORIZATION_CONTRACT, "operations": module.OPERATIONS,
        ...
        "formal_database": {"path": str(database), "device": database.stat().st_dev, "inode": database.stat().st_ino},
        "actor": args.actor, "reason": args.reason, "issued_at": at,
        "user_instruction": args.user_instruction, "source_thread_id": args.source_thread_id})
    successor = {"contract": module.CONTRACT, "transition": module.TRANSITION,
        ...
        "business_e2e": "required", "authorization": authorization_ref,
        "schema_migration_repeated": False, "provider_qualification_repeated": False,
        "database_writes": 0, "paid_gates_reopened": False, "business_scope_change": "explicit_profile_operation",
        ...
        "inherited_publisher_capacity_proof_sha256": inherited["publisher_capacity_proof"]["proof_sha256"]}
    build = {**parent, "source_root": str(args.source_root), "git": tree["git"],
        ...
        "account_profile_successor": successor, "created_at": at,
        "validation_scope": "explicit profile operation authorized by current user instruction; all prior proofs inherited unchanged"}
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fixture.chmod(0o600)
        bootstrap = base.load(args.source_root / "src/dcar_eval/v8/runtime_paths.py", "account_profile_frozen_bootstrap")
    ...
    result = {"contract": "account-profile-install-proposal-v1", "status": "prepared", "created_at": at,
        ...
        "account_profile_proof_sha256": proof["account_profile_proof"]["proof_sha256"],
        "inherited_publisher_capacity_proof_sha256": proof["publisher_capacity_proof"]["proof_sha256"],
        "profile_operation_authority_sha256": proof["profile_operation_authority"]["proof_sha256"],
        "authorization": authorization_ref,
        "database_writes": 0, "provider_calls": 0, "services_changed": False, "paid_gates_reopened": False,
        "schema_migration_repeated": False, "business_scope_change": "explicit_profile_operation"}
