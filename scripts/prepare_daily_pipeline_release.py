#!/usr/bin/env python3
"""Prepare a daily-pipeline successor retaining every historical proof; no DB/service/provider writes.
...
def prepare(args):
    ...
    build = {**parent, "source_root": str(args.source_root), "git": tree["git"],
        ...
        "daily_pipeline_successor": successor, "created_at": at,
        "validation_scope": "reviewed daily pipeline delta; complete recovery, profile, Publisher, controls, metric, catalog and historical operator proofs inherited"}
    ...
    result = {"contract": "daily-pipeline-install-proposal-v1", "status": "prepared", "created_at": at,
        ...
        **{field: proof[name]["proof_sha256"] for field, name in module.INHERITED_PROOFS.items()},
        "daily_pipeline_proof_sha256": proof["daily_pipeline_proof"]["proof_sha256"],
