#!/usr/bin/env python3
"""Prepare a metric/catalog-preserving controls successor; no DB/service/provider writes.
...
def packaging(checkout):
    ...
    spec = importlib.util.spec_from_file_location("controls_packaging_primitives", path)
...
def prepare(args):
    ...
    build = {**parent, "source_root": str(args.source_root), "git": tree["git"],
        ...
        "control_simplification_successor": successor, "created_at": at,
        "validation_scope": "reviewed controls code-only delta; complete metric, catalog and historical operator proofs inherited"}
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fixture.chmod(0o600)
        bootstrap = base.load(args.source_root / "src/dcar_eval/v8/runtime_paths.py", "controls_frozen_bootstrap")
    ...
    result = {"contract": "control-simplification-install-proposal-v1", "status": "prepared", "created_at": at,
        ...
        "control_simplification_proof_sha256": proof["control_simplification_proof"]["proof_sha256"],
