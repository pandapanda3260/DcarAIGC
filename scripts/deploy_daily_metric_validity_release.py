#!/usr/bin/env python3
"""Deploy the reviewed daily pipeline proposal through the pinned catalog deployer.
...
inherited unchanged. The daily successor preserves every historical proof and authorization.
"""
...
def _load_pinned(path=DEPLOYER_SOURCE_PATH, expected_sha256=DEPLOYER_SOURCE_SHA256):
    ...
    spec = importlib.util.spec_from_file_location("daily_pipeline_pinned_catalog_deployer", path)
...
EXPECTED_PARENT_BUILD_SHA256 = "f5851e8c94031f7d5bd56784840b45fb57eb83df344a1203c5cee9b3eeb92005"
...
class Deployer(base.Deployer):
    def policy_proof(self):
        verifier = base.load_verified_module(self.child, "src/dcar_eval/v8/daily_pipeline_release.py",
                                            "daily_pipeline_deployer_lineage")
        ...
        bindings = {
            ...
            "inherited_previous_daily_pipeline_proof_sha256": ("previous_daily_pipeline_proof", self.parent["daily_pipeline_successor"]["parent_build"]),
            "inherited_published_daily_pipeline_proof_sha256": ("published_daily_pipeline_proof", self.proposal["parent_build"]),
            "daily_pipeline_proof_sha256": ("daily_pipeline_proof", self.proposal["child_build"]),
        }
        for field, (name, loaded) in bindings.items():
            require(inherited[name]["proof_sha256"] == self.proposal[field]
                ...
                "Verified daily pipeline proof differs from proposal: " + name)
            if field != "daily_pipeline_proof_sha256":
                require(self.child["daily_pipeline_successor"].get(field) == self.proposal[field],
                    "Daily pipeline proposal and source plan proof differ: " + name)
        require(inherited["previous_daily_pipeline_proof"]["parent_build"].get("sha256") == EXPECTED_RECOVERY_BUILD_SHA256
            and inherited["published_daily_pipeline_proof"]["parent_build"]
                    == self.parent["daily_pipeline_successor"]["parent_build"]
            and inherited["daily_pipeline_proof"]["parent_build"] == self.proposal["parent_build"],
            "Previous or current daily pipeline proof parent differs")
        ...
        require(authority["authorization"] == self.parent["account_profile_successor"]["authorization"]
            ...
            "Published profile authorization changed; daily pipeline cannot issue or renew authority")
        ...
        return {"policy": policy, "policy_sha256": inherited["catalog_capture_policy_sha256"],
                ...
                "previous_daily_pipeline_proof_sha256": inherited["previous_daily_pipeline_proof"]["proof_sha256"],
                "published_daily_pipeline_proof_sha256": inherited["published_daily_pipeline_proof"]["proof_sha256"],
                "daily_pipeline_proof_sha256": inherited["daily_pipeline_proof"]["proof_sha256"]}
    ...
    def preflight(self):
        ...
        require(proposal.get("contract") == "daily-pipeline-install-proposal-v1"
            ...
            "Unexpected daily pipeline proposal, scope or parent/child build")
        ...
        require(isinstance(self.parent.get("daily_pipeline_successor"), dict)
            ...
            and self.parent["daily_pipeline_successor"].get("transition") == "daily-pipeline-20260911-v1"
            and self.parent["daily_pipeline_successor"].get("parent_build", {}).get("sha256") == EXPECTED_PREVIOUS_PARENT_BUILD_SHA256
            and "authorization" not in self.parent["daily_pipeline_successor"]
            and self.child.get("daily_pipeline_successor", {}).get("parent_build") == proposal["parent_build"]
            and self.child["daily_pipeline_successor"].get("contract") == "daily-pipeline-code-successor-v1"
            and self.child["daily_pipeline_successor"].get("transition") == "daily-metric-validity-20260911-v1"
            and "authorization" not in self.child["daily_pipeline_successor"]
            ...
            "Prepared daily pipeline parent or inherited successor/schema lineage differs")
