#!/usr/bin/env python3
"""Prepare a Publisher-capacity successor retaining every historical proof; no DB/service/provider writes.
...
def packaging(checkout):
    ...
    spec = importlib.util.spec_from_file_location("publisher_capacity_packaging_primitives", path)
...
def prepare(args):
    ...
    build = {**parent, "source_root": str(args.source_root), "git": tree["git"],
        ...
        "publisher_capacity_successor": successor, "created_at": at,
        "validation_scope": "reviewed Publisher capacity delta; complete Publisher, controls, metric, catalog and historical operator proofs inherited"}
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fixture.chmod(0o600)
        bootstrap = base.load(args.source_root / "src/dcar_eval/v8/runtime_paths.py", "publisher_capacity_frozen_bootstrap")
    ...
    result = {"contract": "publisher-capacity-install-proposal-v1", "status": "prepared", "created_at": at,
        ...
        "publisher_capacity_proof_sha256": proof["publisher_capacity_proof"]["proof_sha256"],
