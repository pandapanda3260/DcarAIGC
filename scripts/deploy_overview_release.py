#!/usr/bin/env python3
"""Deploy the reviewed overview proposal through the pinned catalog deployer.
...
EXPECTED_PARENT_BUILD_SHA256 = "e5d53ba78459301648fbe080aa26beff810b135edd2f91021ea9bbc1f51931e8"
...
class Deployer(base.Deployer):
    def policy_proof(self):
        verifier = base.load_verified_module(self.child, "src/dcar_eval/v8/overview_release.py",
        ...
        bindings = {
            ...
            "inherited_daily_pipeline_proof_sha256": ("daily_pipeline_proof", self.proposal["parent_build"]),
            "overview_proof_sha256": ("overview_proof", self.proposal["child_build"]),
        }
        for field, (name, loaded) in bindings.items():
            ...
            if field != "overview_proof_sha256":
                require(self.child["overview_successor"].get(field) == self.proposal[field],
        ...
        require(previous["parent_build"].get("sha256") == EXPECTED_RECOVERY_BUILD_SHA256
            ...
            and overview["parent_build"] == self.proposal["parent_build"]
    ...
    def preflight(self):
        ...
        require(proposal.get("contract") == "overview-install-proposal-v1"
        ...
        parent_successors = ("daily_pipeline_successor", "account_profile_recovery_successor", "account_profile_successor",
        ...
        require(isinstance(self.parent.get("daily_pipeline_successor"), dict)
            and self.parent.get("overview_successor") is None
            ...
            and self.child.get("overview_successor", {}).get("parent_build") == proposal["parent_build"]
            and self.child["overview_successor"].get("contract") == "overview-code-successor-v1"
            and self.child["overview_successor"].get("transition") == "overview-four-platforms-20260912-v1"
            and "authorization" not in self.child["overview_successor"]
