#!/usr/bin/env python3
"""Prepare a overview successor retaining every historical proof; no DB/service/provider writes.
...
def prepare(args):
    ...
    build = {**parent, "source_root": str(args.source_root), "git": tree["git"],
        ...
        "overview_successor": successor, "created_at": at,
        "validation_scope": "reviewed overview delta; complete recovery, profile, Publisher, controls, metric, catalog and historical operator proofs inherited"}
    ...
    result = {"contract": "overview-install-proposal-v1", "status": "prepared", "created_at": at,
        ...
        **{field: proof[name]["proof_sha256"] for field, name in module.INHERITED_PROOFS.items()},
        "overview_proof_sha256": proof["overview_proof"]["proof_sha256"],
