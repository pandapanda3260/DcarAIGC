#!/usr/bin/env python3
"""Prepare a controls/metric/catalog-preserving Publisher successor; no DB/service/provider writes.
...
def packaging(checkout):
    ...
    spec = importlib.util.spec_from_file_location("publisher_snapshot_packaging_primitives", path)
...
def prepare(args):
    ...
    build = {**parent, "source_root": str(args.source_root), "git": tree["git"],
        ...
        "publisher_snapshot_successor": successor, "created_at": at,
        "validation_scope": "reviewed Publisher code-only delta; complete controls, metric, catalog and historical operator proofs inherited"}
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fixture.chmod(0o600)
        bootstrap = base.load(args.source_root / "src/dcar_eval/v8/runtime_paths.py", "publisher_snapshot_frozen_bootstrap")
    ...
    result = {"contract": "publisher-snapshot-install-proposal-v1", "status": "prepared", "created_at": at,
        ...
        "publisher_snapshot_proof_sha256": proof["publisher_snapshot_proof"]["proof_sha256"],
