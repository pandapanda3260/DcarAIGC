...
from tests import test_publisher_snapshot_release as publisher_fixtures
...
DISPATCH = '''    if build.get("publisher_capacity_successor") is not None:
        import importlib.util
...
class PublisherCapacityReleaseTest(unittest.TestCase):
    def setUp(self):
        ...
        self.publisher_fixture = c = publisher_fixtures.PublisherSnapshotReleaseTest()
        ...
        self.source = f.root / "capacity-source"
        shutil.copytree(c.source, self.source)
        changes = {}
        for name in ("deploy/macos/publish_snapshot.py", "src/dcar_eval/v8/account_classification_release.py"):
            ...
            if name.endswith("publish_snapshot.py"):
                ...
                marker = '    if build.get("publisher_snapshot_successor") is not None:\n'
        ...
        tree_ref = self.write("capacity-tree.json", self.tree)
        ...
        plan = self.write("capacity-source-plan.json", {"contract": "account-cleanup-source-plan-v1",
        ...
        self.build = {**self.parent, "source_root": str(self.source), "git": self.tree["git"],
            ...
            "publisher_capacity_successor": {"contract": self.module.CONTRACT, "transition": self.module.TRANSITION,
        ...
        self.home = f.root / "capacity-home"
    ...
    def seal(self):
        self.ref = self.write("capacity-build.json", {"contract_version": "sealed-build-receipt-v1",
    ...
    def test_bootstrap_retains_complete_controls_metric_catalog_and_legacy_proof_without_writes(self):
        ...
        result = self.verify()
        self.assertEqual({k: v for k, v in result.items() if k != "publisher_capacity_proof"}, self.baseline)
        ...
        self.assertEqual(result["publisher_snapshot_proof"]["loaded_build"], self.parent_ref)
        self.assertEqual(self.build["publisher_snapshot_successor"], self.parent["publisher_snapshot_successor"])
        self.assertEqual(result["publisher_capacity_proof"]["loaded_build"], self.ref)
    ...
    def test_controls_proof_reference_must_match_complete_inheritance(self):
        self.build["publisher_capacity_successor"]["inherited_control_simplification_proof_sha256"] = "0" * 64
        self.seal()
    ...
    def test_publisher_proof_reference_must_match_complete_inheritance(self):
        self.build["publisher_capacity_successor"]["inherited_publisher_snapshot_proof_sha256"] = "0" * 64
        self.seal()
    ...
    def test_release_cannot_reopen_gates_migrate_or_claim_transport_qualification(self):
        ...
        for key, value in (("paid_gates_reopened", True), ("schema_migration_repeated", True),
            ...
            self.build = copy.deepcopy(original)
            self.build["publisher_capacity_successor"][key] = value
            self.seal()
    ...
    def test_missing_checks_changed_log_or_catalog_proof_are_rejected(self):
        original = copy.deepcopy(self.build)
        self.build["publisher_capacity_successor"]["checks"] = {}
        self.seal()
        with self.assertRaisesRegex(ValueError, "checks are incomplete"):
            self.verify()
        self.build = copy.deepcopy(original)
        self.build["publisher_capacity_successor"]["inherited_catalog_proof_sha256"] = "0" * 64
        self.seal()
        with self.assertRaisesRegex(ValueError, "catalog proof differs"):
            self.verify()
        self.build = original
        self.seal()
        report = self.module.object_at(next(iter(self.build["publisher_capacity_successor"]["checks"].values())))
    ...
    def test_parent_verifier_is_pinned_and_unreviewed_child_bytes_fail_bootstrap(self):
        ...
        self.publisher_fixture.bootstrap()
        self.bootstrap()
    ...
    def test_inherited_proofs_cannot_be_omitted_or_point_to_another_build(self):
        for proof_key in ("publisher_snapshot_proof", "control_simplification_proof"):
    ...
    def test_latest_parent_publisher_check_tamper_cannot_be_skipped(self):
        report = self.module.object_at(next(iter(self.parent["publisher_snapshot_successor"]["checks"].values())))
    ...
    def _packaging_fixture(self):
        def load(path, name):
            ...
            return result
        package = load(ROOT / "scripts/prepare_publisher_capacity_release.py", "fixture_controls_packaging")
    ...
    def test_packaging_failed_check_records_failure_without_preparing_release(self):
        ...
        output = self.fixture.root / "capacity-failed-check.json"
        args = SimpleNamespace(checkout=self.source, parent_build=Path(self.parent_ref["path"]),
            installed_plist=self.parent_plist, name="publisher_capacity_release", output=output,
    ...
    def test_packaging_emits_only_source_receipts_and_paired_plist_proposals(self):
        ...
        args = SimpleNamespace(checkout=self.source, parent_build=Path(self.parent_ref["path"]),
            ...
            source_root=self.fixture.root / "packaged-capacity-source", evidence_root=self.fixture.root / "capacity-evidence",
