...
PATH = Path(__file__).resolve().parents[1] / "scripts/deploy_daily_metric_validity_release.py"
SPEC = importlib.util.spec_from_file_location("daily_pipeline_deploy_under_test", PATH)
...
class DailyPipelineDeployAdapterTest(unittest.TestCase):
    def setUp(self):
        ...
        self.child = {**copy.deepcopy(self.parent), "source_root": str(self.root / "child-source"),
            "daily_pipeline_successor": {"contract": "daily-pipeline-code-successor-v1", "parent_build": self.parent_ref,
        ...
        self.proposal = {"contract": "daily-pipeline-install-proposal-v1", "status": "prepared",
            ...
            "account_profile_recovery_proof_sha256": "1" * 64, "profile_compensation_authority_sha256": "2" * 64,
            "inherited_metric_gap_proof_sha256": "3" * 64, "inherited_manual_content_scope_proof_sha256": "4" * 64,
            "inherited_account_classification_proof_sha256": "5" * 64, "daily_pipeline_proof_sha256": "6" * 64,
            "inherited_previous_daily_pipeline_proof_sha256": "7" * 64,
            "inherited_published_daily_pipeline_proof_sha256": "8" * 64,
        ...
        for name, digit, ancestor in (("catalog_capture_proof", "a", self.catalog_ref),
                ...
                ("published_daily_pipeline_proof", "8", self.parent_ref), ("daily_pipeline_proof", "6", self.child_ref)):
        ...
        self.inherited["daily_pipeline_proof"]["parent_build"] = self.parent_ref
    ...
    def test_daily_preflight_verifies_full_proof_chain_and_preserves_configuration(self):
        before = self.database.read_bytes(), self.writer.read_bytes(), self.publisher.read_bytes()
        ...
        self.assertEqual(self.module_loader.call_args.args[1], "src/dcar_eval/v8/daily_pipeline_release.py")
    ...
    def test_proposal_gate_change_or_old_contract_fails_before_stop(self):
        for key, value in (("contract", "account-catalog-capture-install-proposal-v1"), ("paid_gates_reopened", True)):
            with self.subTest(key=key):
                original = copy.deepcopy(self.proposal)
                self.proposal[key] = value
                self.save_proposal()
                with self.assertRaisesRegex(ValueError, "Unexpected daily pipeline proposal"):
                    self.instance.run()
    ...
    def test_child_parent_and_all_nine_historical_successors_are_preserved(self):
        names = [key for key in self.parent if key.endswith("_successor") and key != "daily_pipeline_successor"]
        self.assertEqual(len(names), 9)
        for key in ("parent", *names):
            with self.subTest(key=key):
                child = copy.deepcopy(self.child)
                if key == "parent":
                    child["daily_pipeline_successor"]["parent_build"]["sha256"] = "0" * 64
                else:
    ...
    def test_all_fourteen_proof_hashes_are_independently_bound(self):
        self.assertEqual(len(self.proof_names), 14)
        for key in self.proof_names:
            with self.subTest(key=key):
    ...
    def test_new_proposal_or_successor_authorization_is_rejected(self):
        ...
        child = copy.deepcopy(self.child)
        child["daily_pipeline_successor"]["authorization"] = self.compensation_authorization
        self.proposal["child_build"] = self.build("child-build.json", child)
        self.instance.args.expected_child_build = self.proposal["child_build"]["sha256"]
        self.save_proposal()
        with self.assertRaisesRegex(ValueError, "inherited successor/schema lineage"):
    ...
    def test_all_proofs_reject_every_other_generation(self):
        ancestors = (self.classification_ref, self.manual_ref, self.catalog_ref, self.metric_ref, self.controls_ref,
                     self.publisher_ref, self.capacity_ref, self.profile_ref, self.recovery_ref, self.daily_v2_ref, self.parent_ref, self.child_ref)
        for key in self.proof_names:
            original = self.inherited[key]["loaded_build"]
    ...
    def test_reviewed_real_parent_and_its_previous_parent_are_pinned(self):
        self.assertEqual(REAL_PARENT_SHA256, "f5851e8c94031f7d5bd56784840b45fb57eb83df344a1203c5cee9b3eeb92005")
    ...
    def test_previous_and_current_daily_proofs_preserve_distinct_parents(self):
        ...
        for name, original, wrong in (
                ...
                ("daily_pipeline_proof", self.parent_ref, self.recovery_ref),
                ("daily_pipeline_proof", self.parent_ref, self.child_ref)):
            with self.subTest(proof=name, wrong=wrong["path"]):
                self.inherited[name]["parent_build"] = wrong
                with self.assertRaisesRegex(ValueError, "proof parent differs|proof differs"):
                    self.instance.preflight()
    ...
    def test_proposal_cannot_overwrite_any_inherited_source_plan_proof(self):
        for field in self.proof_fields:
            with self.subTest(field=field):
                ...
                changed["daily_pipeline_successor"][field] = "0" * 64
                ...
                self.inherited["daily_pipeline_proof"]["loaded_build"] = child_ref
                self.save_proposal()
