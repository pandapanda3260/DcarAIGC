...
SPEC = importlib.util.spec_from_file_location(
    "overview_release_under_test", ROOT / "src/dcar_eval/v8/overview_release.py"
)
...
class OverviewReleaseTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.parent = {"status": "succeeded", "created_at": "2026-09-11T00:00:00+00:00",
            ...
            "daily_pipeline_successor": {"contract": "daily-pipeline-code-successor-v1",
                                         "transition": release.PARENT_TRANSITION},
        ...
        plan = {"contract": release.CONTRACT, "transition": release.TRANSITION,
            ...
            "actor": "test", "reason": "overview", "issued_at": AT,
    ...
    def test_keeps_every_historical_proof_and_authorization_unchanged(self):
        ...
        self.assertEqual(value["overview_proof"]["transition"], release.TRANSITION)
    ...
    def test_rejects_each_historical_proof_hash_change(self):
        for field in release.INHERITED_PROOFS:
            with self.subTest(field=field):
                ...
                build["overview_successor"][field] = "f" * 64
    ...
    def test_rejects_database_migration_paid_or_new_authority_scope(self):
        for field, value in (("database_writes", 1), ("schema_migration_repeated", True),
            ...
            with self.subTest(field=field):
                build = deepcopy(self.build)
                build["overview_successor"][field] = value
                with self.assertRaises(ValueError):
                    self.verify(build)

    def test_rejects_missing_check_and_changed_output(self):
        build = deepcopy(self.build)
        build["overview_successor"]["checks"].pop("overview_behavior")
        with self.assertRaises(ValueError):
            self.verify(build)
        (self.root / "overview_behavior.log").write_text("tampered output")
        with self.assertRaises(ValueError):
            self.verify()
    ...
    def test_rejects_future_issuance_and_changed_transition(self):
        build = deepcopy(self.build)
        build["overview_successor"]["transition"] = release.PARENT_TRANSITION
        with self.assertRaises(ValueError):
            self.verify(build)
        self.build["created_at"] = self.build["overview_successor"]["issued_at"] = "2100-01-01T00:00:00Z"
        with self.assertRaises(ValueError):
            self.verify()
