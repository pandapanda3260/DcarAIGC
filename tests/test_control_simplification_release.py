...
Run with the candidate on PYTHONPATH. The real metric successor fixture creates
temporary DBs/receipts, including the catalog and historical schema lineage.
The controls verification under test performs no migration or database writes.
"""
...
from tests import test_metric_gap_release as metric_fixtures
...
AT = metric_fixtures.AT
DISPATCH = '''    if build.get("control_simplification_successor") is not None:
        import importlib.util
...
class ControlSimplificationReleaseTest(unittest.TestCase):
    def setUp(self):
        ...
        self.metric_fixture = c = metric_fixtures.MetricGapReleaseTest()
        ...
        self.source = f.root / "controls-source"
        shutil.copytree(c.source, self.source)
        changes = {}
        for name in ("src/dcar_eval/v8/api.py", "src/dcar_eval/v8/account_classification_release.py"):
            path = self.source / name
            old = path.read_bytes()
            if name.endswith("api.py"):
                new = old + b"\n# synthetic reviewed controls fixture\n"
            else:
                marker = '    if build.get("metric_gap_successor") is not None:\n'
        ...
        tree_ref = self.write("controls-tree.json", self.tree)
        ...
        plan = self.write("controls-source-plan.json", {"contract": "account-cleanup-source-plan-v1",
        ...
        self.build = {**self.parent, "source_root": str(self.source), "git": self.tree["git"],
            "critical_files": {r["path"]: r["sha256"] for r in self.tree["files"]}, "code_successor_plan": plan,
            "account_cleanup_generation": {**self.parent["account_cleanup_generation"], "source_tree": tree_ref},
            "control_simplification_successor": {"contract": self.module.CONTRACT, "transition": self.module.TRANSITION,
        ...
        self.seal()
        env = plistlib.loads(c.plist.read_bytes())["EnvironmentVariables"]
        env = {**env, "DCAR_LOADED_BUILD_RECEIPT": self.ref["path"], "DCAR_WRITER_SOURCE_ROOT": str(self.source)}
        self.home = f.root / "controls-home"
    ...
    def seal(self):
        self.ref = self.write("controls-build.json", {"contract_version": "sealed-build-receipt-v1",
    ...
    def test_bootstrap_retains_complete_metric_catalog_and_legacy_proof_without_writes(self):
        self.assertEqual(self.bootstrap()["files"], len(self.tree["files"]))
        result = self.verify()
        self.assertEqual({k: v for k, v in result.items() if k != "control_simplification_proof"}, self.baseline)
    ...
    def test_catalog_schema_operator_and_database_authority_cannot_change(self):
        ...
        for key in ("catalog", "metric", "schema", "operator", "migration"):
    ...
    def test_release_cannot_reopen_gates_migrate_or_claim_transport_qualification(self):
        ...
        for key, value in (("paid_gates_reopened", True), ("schema_migration_repeated", True),
            ...
            self.build = copy.deepcopy(original)
            self.build["control_simplification_successor"][key] = value
            self.seal()
    ...
    def test_missing_checks_changed_log_or_catalog_proof_are_rejected(self):
        original = copy.deepcopy(self.build)
        self.build["control_simplification_successor"]["checks"] = {}
        self.seal()
        with self.assertRaisesRegex(ValueError, "checks are incomplete"):
            self.verify()
        self.build = copy.deepcopy(original)
        self.build["control_simplification_successor"]["inherited_catalog_proof_sha256"] = "0" * 64
        self.seal()
        with self.assertRaisesRegex(ValueError, "catalog proof differs"):
            self.verify()
        self.build = original
        self.seal()
        report = self.module.object_at(next(iter(self.build["control_simplification_successor"]["checks"].values())))
    ...
    def test_metric_inheritance_cannot_be_omitted_or_point_to_another_build(self):
        for key in ("missing", "wrong_build"):
            inherited = copy.deepcopy(self.baseline)
            if key == "missing":
                inherited.pop("metric_gap_proof")
            else:
                inherited["metric_gap_proof"]["loaded_build"]["sha256"] = "0" * 64
            ...
            with patch.object(self.module, "_load", return_value=verifier):
                with self.assertRaisesRegex(ValueError, "complete verified metric inheritance"):
                    self.verify()

    def test_latest_parent_metric_check_tamper_cannot_be_skipped(self):
        report = self.module.object_at(next(iter(self.parent["metric_gap_successor"]["checks"].values())))
        Path(report["output"]["path"]).write_text("changed latest parent metric log\n")
        with self.assertRaisesRegex(ValueError, "check output changed"):
            self.verify()

    def test_child_cannot_replace_historical_metric_verifier(self):
        for name in (self.module.CATALOG_MODULE, self.module.METRIC_MODULE):
            ...
            with self.assertRaisesRegex(ValueError, "published metric and catalog verifiers"):
    ...
    def _packaging_fixture(self):
        def load(path, name):
            ...
            return result
        package = load(ROOT / "scripts/prepare_control_simplification_release.py", "fixture_controls_packaging")
        owner = Path(metric_fixtures.__file__).resolve().parents[1]
    ...
    def test_packaging_failed_check_records_failure_without_preparing_release(self):
        ...
        output = self.fixture.root / "controls-failed-check.json"
        args = SimpleNamespace(checkout=self.source, parent_build=Path(self.parent_ref["path"]),
            installed_plist=self.metric_fixture.plist, name="controls_release", output=output,
    ...
    def test_packaging_emits_only_source_receipts_and_paired_plist_proposals(self):
        ...
        for name in sorted(self.module.REQUIRED_CHECKS):
            ...
            args = SimpleNamespace(checkout=self.source, parent_build=Path(self.parent_ref["path"]),
                installed_plist=self.metric_fixture.plist, name=name, output=output,
        ...
        writer_before, publisher_before = self.metric_fixture.plist.read_bytes(), publisher.read_bytes()
        args = SimpleNamespace(checkout=self.source, parent_build=Path(self.parent_ref["path"]),
            installed_plist=self.metric_fixture.plist, publisher_plist=publisher,
            source_root=self.fixture.root / "packaged-controls-source", evidence_root=self.fixture.root / "controls-evidence",
        ...
        self.assertEqual(self.metric_fixture.plist.read_bytes(), writer_before)
