...
def _require_schema19(connection: sqlite3.Connection) -> None:
    if int(connection.execute("PRAGMA user_version").fetchone()[0]) not in {19, 20, 21, 22, 23, 24}:
        raise ProfileControlError(
...
def begin_cross_profile_switch_in_transaction(
    ...
    *,
    ...
    _require_schema19(connection)
    if effective_now and connection.execute("PRAGMA user_version").fetchone()[0] in {20, 21, 22, 23, 24}:
