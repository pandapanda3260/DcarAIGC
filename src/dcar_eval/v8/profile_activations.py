...
def _validate_chain(connection: sqlite3.Connection) -> list[dict[str, Any]]:
    ...
    for value in rows:
        ...
        if (
            ...
            or (value["profile_id"] == INTEGRATED_PROFILE and schema_version not in {20, 21, 22, 23, 24})
    ...
    for row in connection.execute("SELECT * FROM activation_cancellations ORDER BY id"):
        ...
        if (
            value["contract_version"] not in ({CANCELLATION_CONTRACT_VERSION, CANCELLATION_V2}
                                               if schema_version in {20, 21, 22, 23, 24} else {CANCELLATION_CONTRACT_VERSION})
...
def activation_eligibility(connection: sqlite3.Connection, activation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    required = (connection.execute("PRAGMA user_version").fetchone()[0] in {20, 21, 22, 23, 24}
...
def append_activation(
    ...
    *,
    ...
    if profile_id == INTEGRATED_PROFILE and int(connection.execute("PRAGMA user_version").fetchone()[0]) not in {20, 21, 22, 23, 24}:
