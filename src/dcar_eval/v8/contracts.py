...
CURRENT_REPORT_VERSION = "dcar-content-operations-report-v8.0"
...
def expected_terminal_task_status(data_quality: Mapping[str, Any]) -> str:
    contract = load_contract()
    thresholds = contract["required_coverage_thresholds"]
...
def _validate_metric(metric: Any, expected: Mapping[str, str], path: str, errors: List[str]) -> None:
    contract = load_contract()
    ...
    if kind == "quantity":
        ...
        required = {
            "numerator", "denominator", "percentage", "unit", "status",
            "eligible_count", "coverage_percentage", "reason",
        }
        ...
        if status in {"not_calculable", "not_applicable", "missing"} and metric.get("percentage") is not None:
            errors.append(f"{path}.percentage must be null for status {status}")
    else:
        required = {
            "value", "unit", "scale", "status", "scorable_items",
            "total_items", "coverage_percentage", "reason",
        }
...
def validate_report(report: Mapping[str, Any]) -> None:
    ...
    if not isinstance(taxonomy_version, str) or re.fullmatch(
        contract["taxonomy_version_pattern"], taxonomy_version
    ) is None:
    ...
    if not isinstance(data_quality, Mapping):
        ...
        if task_status in {"succeeded", "partial"}:
            expected_status = expected_terminal_task_status(data_quality)
    ...
    if not isinstance(summary, Mapping):
        ...
        for name, expected in contract["summary_metrics"].items():
            _validate_metric(summary.get(name), expected, f"$.summary_metrics.{name}", errors)
        ...
        if isinstance(publication_value, (int, float)):
            for name in (
                "verticality_rate", "selling_point_coverage_rate", "estimated_new_user_rate",
                "estimated_reactivation_rate", "estimated_lead_rate",
            ):
    ...
    for key in (
        "platform_dimensions", "account_type_dimensions", "content_direction_dimensions",
        "selling_point_dimensions", "duplicates", "review_summary", "capture_summary",
        "provider_costs", "content_details", "files",
    ):
